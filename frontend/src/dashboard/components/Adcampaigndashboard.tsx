...
const GLOBAL_CSS = `
  ...
  .acd-toast {
    ...
    color: #FFFFFF;
    padding: 10px 18px;
    border: 1.5px solid #00c788cc;
    border-radius: 12px;
    font-size: 12px;
    font-weight: 600;
    box-shadow: 0 8px 32px rgba(37, 99, 235, 0.35);
    animation: acd-fi .2s ease;
    display: flex;
    align-items: center;
    gap: 8px;
  }
    ...
    <div className="fb-post">
      <div style={{ padding: "12px 14px 8px", display: "flex", alignItems: "center", gap: 9 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: "#8A97B0", display: "flex", alignItems: "center", gap: 4 }}>Sponsored · 🌐</div>
        </div>
...
function GooglePreview({ brandName, caption, cta, imageUrl }: PreviewProps) {
  return (
    <div className="google-ad">
      ...
      <div style={{ fontSize: 16, color: "#1558D6", fontWeight: 500, marginBottom: 4, lineHeight: 1.4 }}>{caption?.slice(0, 60) || "Powerful Solutions for Every Business — Official Site"}</div>
      <div style={{ fontSize: 12, color: "#4A5878", lineHeight: 1.6, marginBottom: 12 }}>{caption || "Discover our latest campaign — built for results. Affordable and eco-friendly solutions."}</div>
    ...
    <div className="x-post">
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, color: "#8A97B0" }}>
            {["💬 24", "🔁 89", "❤️ 412", "📤"].map(a => (
...
const sLabel = (color = "var(--t2)"): React.CSSProperties => ({
  ...
  letterSpacing: ".7px", marginBottom: 10, display: "flex", alignItems: "center", gap: 6,
});
...
function Sidebar({ platforms, campaigns, activePlatformId, activeCampaignId, onPlatformSwitch, onSelectCampaign, onAddCampaign }: SidebarProps) {
    ...
    <aside className="dash-sidebar">
          ...
          {platforms.map(p => {
            ...
            return (
              <div key={p.id} className={`plat-tab${active ? " nav-active" : ""}`} onClick={() => onPlatformSwitch(p.id)}
                ...
                <div style={{ width: 28, height: 28, borderRadius: 8, background: active ? p.bg : "var(--surface2)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .15s" }}>{p.icon}</div>
                <span style={{ fontSize: 10, color: active ? p.color : "var(--t3)", fontWeight: active ? 700 : 400, letterSpacing: ".3px" }}>{p.name}</span>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 10px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 6px", marginBottom: 8 }}>
          ...
          <button className="add-btn" onClick={() => onAddCampaign(activePlatformId)}
            style={{ width: 20, height: 20, borderRadius: 5, border: "1px solid var(--bdr)", background: "var(--surface2)", color: "var(--t3)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0, transition: "all .15s" }}>
            <I.Plus />
          </button>
        </div>
        {aCamps.length === 0
...
/* ─── TOP BAR ─────────────────────────────────────────────── */
interface TopBarProps { activePlatformId: PlatformId; }
...
function TopBar({ activePlatformId }: TopBarProps) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, padding: "10px 16px", borderBottom: "1px solid var(--bdr)", background: "#fff", alignItems: "flex-end", flexWrap: "wrap", flexShrink: 0, position: "relative" }}>
      {fields.map(f => (
        <div key={f.label} style={{ display: "flex", flexDirection: "column", gap: 3, flex: 1, minWidth: 110, opacity: en ? 1 : .35, pointerEvents: en ? "auto" : "none", transition: "opacity .2s" }}>
...
function CreativeStudio({ brandName, adCopy, activePlatformId, brandAssetImages, onHeadingChange, onSubheadingChange, onImageSelect, onCtaChange }: CreativeStudioProps) {
  ...
  const upload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadedImgs(p => [url, ...p].slice(0, 6)); pickImg(url);
    setImgTab("upload");
  };
          ...
          <div style={{ display: "flex", gap: 5, marginBottom: 8, flexWrap: "wrap" }}>
            {adCopy.primaryTexts.map((s, i) => (
              <button key={i} className={`tag-pill${sIdx === i ? " on" : ""}`} onClick={() => pickS(s, i)}
                style={{ fontSize: 10, padding: "3px 10px", borderRadius: 20, border: "1px solid var(--bdr)", background: "var(--surface2)", color: "var(--t2)", fontFamily: "inherit", transition: "all .12s" }}>P{i + 1}</button>
            ))}
          </div>
          <textarea className="pt-ta" value={sub}
            onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => { setSub(e.target.value); onSubheadingChange(e.target.value); }}
            rows={3}
              ...
              {brandAssetImages.length === 0 ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
                  {brandAssetImages.map((url, i) => (
                    <div key={i} className={`brand-asset-img${selImg === url ? " sel" : ""}`} onClick={() => pickImg(url)}
                      style={{ aspectRatio: "1", position: "relative", overflow: "hidden", border: `2px solid ${selImg === url ? "var(--blue)" : "var(--bdr)"}`, cursor: "pointer" }}>
          ...
          {imgTab === "ai" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ background: "var(--purple-lt)", border: "1px solid var(--purple-bdr)", borderRadius: 10, padding: 12 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--purple)", marginBottom: 7, display: "flex", alignItems: "center", gap: 5, textTransform: "uppercase", letterSpacing: ".5px" }}>
                  <I.Sparkle /> AI Image Generator
                </div>
                <textarea className="pt-ta" value={aiPrompt} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setAiPrompt(e.target.value)}
                  placeholder={`e.g. "Eco-friendly solar panels on a rooftop at golden hour"`}
                  rows={2}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
                  ...
                  {aiImgs.map((url, i) => (
                    <div key={i} className={`img-th${selImg === url ? " sel" : ""}`} onClick={() => pickImg(url)}
                      style={{ aspectRatio: "1", borderRadius: 7, overflow: "hidden", border: `2px solid ${selImg === url ? "var(--blue)" : "var(--bdr)"}`, position: "relative" }}>
                      <img src={url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      {selImg === url && <div style={{ position: "absolute", top: 4, right: 4, width: 16, height: 16, borderRadius: "50%", background: "var(--blue)", display: "flex", alignItems: "center", justifyContent: "center" }}><I.Check /></div>}
                    </div>
                  ))}
                </div>
              )}
          ...
          {imgTab === "upload" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button onClick={() => fileRef.current?.click()}
                style={{ width: "100%", background: "var(--surface)", border: "1.5px dashed var(--bdr2)", borderRadius: 9, padding: "14px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, cursor: "pointer", color: "var(--t2)", fontSize: 12, fontWeight: 600, fontFamily: "inherit", transition: "background .12s" }}>
  ...
  const plans: Plan[] = [
    {
      ...
      features: ["1 campaign/month", "Basic analytics", "Email support", "Standard publishing", "Limited AI images"],
      color: "#64748b",
    },
    {
      id: "silver", name: "Silver", price: billing === "monthly" ? "$29" : "$290",
      features: ["10 campaigns/month", "Advanced analytics", "Priority support", "Scheduled publishing", "Unlimited AI images", "A/B testing"],
      color: "#2563EB", popular: true,
    },
    {
      id: "gold", name: "Gold", price: billing === "monthly" ? "$79" : "$790",
      features: ["Unlimited campaigns", "Real-time analytics", "24/7 support", "Advanced scheduling", "Unlimited AI images", "A/B testing", "Multi-platform", "Custom integrations"],
      color: "#D97706",
    },
  ];
          ...
          <div style={{ display: "flex", background: "var(--surface2)", borderRadius: 40, padding: 4, border: "1px solid var(--bdr)" }}>
            {(["monthly", "yearly"] as BillingCycle[]).map(b => (
              <button key={b} onClick={() => setBilling(b)}
                style={{ padding: "7px 22px", borderRadius: 32, border: "none", background: billing === b ? "var(--blue)" : "transparent", color: billing === b ? "#fff" : "var(--t2)", fontWeight: 600, fontSize: 13, cursor: "pointer", transition: "all .2s", fontFamily: "inherit", position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 24 }}>
          {plans.map(plan => (
            <div key={plan.id} className="plan-card" onClick={() => onSelectPlan(plan.id)}
              style={{ border: `${plan.popular ? "2px" : "1px"} solid ${plan.popular ? plan.color + "44" : "var(--bdr)"}`, borderRadius: 16, padding: 22, background: plan.popular ? `${plan.color}08` : "var(--surface)", cursor: "pointer", transition: "all .2s", position: "relative" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 12, color: "var(--t3)" }}>/{billing === "monthly" ? "mo" : "yr"}</span>
                </div>
              </div>
              ...
              <button style={{ width: "100%", padding: "9px", borderRadius: 8, border: `1.5px solid ${plan.color}`, background: plan.popular ? plan.color : "transparent", color: plan.popular ? "#fff" : plan.color, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit", transition: "all .2s" }}>
                Select {plan.name}
              </button>
            </div>
          ))}
...
function BottomBar({ onBack, onPublish, onSaveDraft, loading, activePlatformName }: BottomBarProps) {
  return (
    <div style={{ background: "#fff", borderTop: "1px solid var(--bdr)", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
      <button className="btn-back" onClick={onBack} style={{ background: "var(--surface)", border: "1px solid var(--bdr)", color: "var(--t2)", padding: "8px 16px", borderRadius: 9, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6, transition: "all .12s" }}>
        <I.Back /> Back
      </button>
  ...
  return (
    <>
      ...
      <div className="dash-root">
        ...
        <TopBar activePlatformId={activePid} />
        ...
        <div className="dash-inner">
          ...
          <div className="dash-main">
            ...
            <BottomBar
              onBack={onBack} onPublish={handlePublish} onSaveDraft={handleDraft}
