...
const ConnectPlatformModal: React.FC<ConnectPlatformModalProps> = ({ connectedAccounts, onConnect, onDisconnect, onClose }) => {
  ...
  const startOAuth = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/auth/oauth-url/${selectedPlat}`, { headers: { Authorization: `Bearer ${localStorage.getItem('access_token')||''}` } });
  ...
  const saveManual = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/auth/connect-platform`, {
        method: 'POST',
          ...
          {step === 'manual' && platInfo && (
            <div>
              ...
              <button
                ...
                style={{ width: '100%', padding: 11, borderRadius: 10, border: 'none', background: (!manualId.trim() || !manualToken.trim()) ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg,#0665ff,#1e27a8)', color: (!manualId.trim() || !manualToken.trim()) ? '#475569' : '#fff', cursor: (!manualId.trim() || !manualToken.trim()) ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.88rem' }}
              >
            ...
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              ...
              {connectStatus === 'success' && (
                <>
                  ...
                  <button onClick={onClose} style={{ padding: '10px 28px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#0665ff,#1e27a8)', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem' }}>Done</button>
                </>
              )}
              {connectStatus === 'error' && (
                <>
