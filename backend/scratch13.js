...
async function run() {
  ...
  const account = await db.collection('linkedinaccounts').findOne({});
  if (!account) { console.log('No linkedin account found'); return process.exit(0); }

  console.log('Got account:', account._id);
